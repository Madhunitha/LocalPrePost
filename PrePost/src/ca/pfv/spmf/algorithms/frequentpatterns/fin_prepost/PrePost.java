 ...
 */
/**
 * Java implementation of the PrePost algorithm.
 * 
...
//BufferedWriter writer = null; // object to write the output file
	 ...
	 */
	public Itemsets runAlgorithm(String filename, double minsup, String output)
			throws IOException {
		outputCount = 0;
		nlNodeCount = 0;
			...
			throws IOException
{
		...
		else
		{
			...
			// found.
			int[] itemsetArray = new int[resultLen];
			if(curNode.support >= minSupport) 
			  {
				outputCount++;
				...
				for (int i = 0; i < resultLen; i++) 
				{
			//System.arraycopy(item[result[i]].index, 0, itemsetArray, 0, resultLen);
					itemsetArray[i]=item[result[i]].index;
				}
			  }
			...
			itemsetObj.setAbsoluteSupport(curNode.support);
			patterns.addItemset(itemsetObj, resultLen);
			if (sameCount > 0) 
			  {
				// generate all subsets of the node list except the empty set
				for (long i = 1, max = 1 << sameCount; i < max; i++) 
				{
					for (int k = 0; k < resultLen; k++) 
					{
						//System.arraycopy(item[result[k]].index, 0, itemsetArray, 0, resultLen);
						itemsetArray[k]=item[result[k]].index;
					}
					...
					for (int j = 0; j < sameCount; j++) 
					{
						...
						{
							// if yes, add it to the set
							//System.arraycopy(item[result[j]].index, 0, itemsetArray, 0, resultLen);
							itemsetArray[j]=item[result[j]].index;
						}
					}
					Arrays.sort(itemsetArray);
					
					Itemset itemsetObj1 = new Itemset(itemsetArray);
					itemsetObj1.setAbsoluteSupport(curNode.support);
					patterns.addItemset(itemsetObj1, resultLen);
					outputCount++;
				}
		}
	}
